...
import { TrendingUp, DollarSign, Activity, Percent, Loader2 } from "lucide-react";
...
export default function App() {
  ...
  const [loading, setLoading] = useState({ ipca: true, selic: true, dolar: true });
  ...
  const carregarDados = () => {
    // Ativa o loading ao iniciar a busca
    setLoading({ ipca: true, selic: true, dolar: true });

    fetch(`${API}/ipca`)
      .then((res) => res.json())
      .then((data) => {
        setIpca(data.dados_ultimos_12_meses || []);
        setUpdates((prev) => ({ ...prev, ipca: data.ultima_data_real }));
        setPrevisoes((prev) => ({ ...prev, ipca: data.previsao_proximo_mes }));
      })
      .catch((err) => console.error(err))
      .finally(() => setLoading((prev) => ({ ...prev, ipca: false })));

    fetch(`${API}/selic`)
      .then((res) => res.json())
      .then((data) => {
        setSelic(data.dados_ultimos_12_registros || []);
        setUpdates((prev) => ({ ...prev, selic: data.ultima_data_real }));
        setPrevisoes((prev) => ({ ...prev, selic: data.previsao_proxima_taxa }));
      })
      .catch((err) => console.error(err))
      .finally(() => setLoading((prev) => ({ ...prev, selic: false })));

    fetch(`${API}/dolar`)
      .then((res) => res.json())
      .then((data) => {
        setDolar(data.dados_ultimos_30_dias || []);
        setUpdates((prev) => ({ ...prev, dolar: data.ultima_data_real }));
        setPrevisoes((prev) => ({ ...prev, dolar: data.previsao_proximo_dia }));
      })
      .catch((err) => console.error(err))
      .finally(() => setLoading((prev) => ({ ...prev, dolar: false })));
  };
  ...
  const tendenciaIPCA =
    previsoes.ipca && ultimoIPCA
      ? previsoes.ipca > ultimoIPCA ? "alta" : previsoes.ipca < ultimoIPCA ? "queda" : "estavel"
      : null;

  const tendenciaSelic =
    previsoes.selic !== null && ultimaSelic !== undefined
      ? previsoes.selic > ultimaSelic ? "alta" : previsoes.selic < ultimaSelic ? "queda" : "estavel"
      : null;

  const tendenciaDolar =
    previsoes.dolar !== null && ultimoDolar !== undefined
      ? previsoes.dolar > ultimoDolar ? "alta" : previsoes.dolar < ultimoDolar ? "queda" : "estavel"
      : null;
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col justify-between hover:border-blue-500/40 transition-all">
            <div>
              <div className="flex justify-between items-start mb-4">
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Inflação (IPCA)</p>
                <Percent size={18} className="text-blue-400" />
              </div>

              {loading.ipca ? (
                <div className="flex items-center gap-2 text-slate-400 h-[36px]">
                  <Loader2 className="animate-spin text-blue-500" size={20} />
                  <span className="text-sm font-medium">Conectando à API...</span>
                </div>
              ) : (
              ...
              {!loading.ipca && previsoes.ipca && tendenciaIPCA && (
                <div className="flex items-center gap-2 mt-2 text-xs font-medium">
                  <span className="text-slate-400">Previsão:</span>
                  <div className={`flex items-center gap-1 font-semibold ${tendenciaIPCA === "alta" ? "text-red-400" : tendenciaIPCA === "queda" ? "text-green-400" : "text-slate-400"}`}>
            ...
            <div className="mt-6 pt-3 border-t border-slate-800/50 flex items-center justify-end">
              <span className="text-[12px] px-2 py-1 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">
                {loading.ipca ? "Buscando dados..." : `Atualização API BCB ${updates.ipca}`}
              </span>
            </div>
          </div>

          {/* CARD SELIC */}
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col justify-between hover:border-green-500/40 transition-all">
            <div>
              <div className="flex justify-between items-start mb-4">
              ...
              {!loading.selic && previsoes.selic !== null && tendenciaSelic && (
                <div className="flex items-center gap-2 mt-2 text-xs font-medium">
                  <span className="text-slate-400">Previsão:</span>
                  <div className={`flex items-center gap-1 font-semibold ${tendenciaSelic === "alta" ? "text-red-400" : tendenciaSelic === "queda" ? "text-green-400" : "text-slate-400"}`}>
            ...
            <div className="mt-6 pt-3 border-t border-slate-800/50 flex items-center justify-end">
              <span className="text-[12px] px-2 py-1 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">
                {loading.selic ? "Buscando dados..." : `Atualização API BCB ${updates.selic}`}
              </span>
            </div>
          </div>

          {/* CARD DÓLAR */}
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col justify-between hover:border-yellow-500/40 transition-all">
            <div>
              <div className="flex justify-between items-start mb-4">
              ...
              {!loading.dolar && previsoes.dolar !== null && tendenciaDolar && (
                <div className="flex items-center gap-2 mt-2 text-xs font-medium">
                  <span className="text-slate-400">Previsão:</span>
                  <div className={`flex items-center gap-1 font-semibold ${tendenciaDolar === "alta" ? "text-red-400" : tendenciaDolar === "queda" ? "text-green-400" : "text-slate-400"}`}>
            ...
            <div className="mt-6 pt-3 border-t border-slate-800/50 flex items-center justify-end">
              <span className="text-[12px] px-2 py-1 rounded-full bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">
                {loading.dolar ? "Buscando dados..." : `Atualização API BCB ${updates.dolar}`}
              </span>
            </div>
          </div>
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={ipca}>
                  ...
                  <XAxis dataKey="data" stroke="#475569" fontSize={11} tickLine={false} axisLine={false} dy={10} />
